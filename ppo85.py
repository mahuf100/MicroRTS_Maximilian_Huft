...
experiment_name = "MicrortsDefeatCoacAIShaped-v3__neu__1__1750993153"
...
envs = MicroRTSGridModeVecEnv(
    ...
    max_steps=2000,
    render_theme=1,
    ai2s=[microrts_ai.passiveAI],# for _ in range(args.num_bot_envs-8)] +
    ...
    reward_weight=np.array([10.0,1.0, 1.0, 0.2, 1.0,0, 5.25, 0, 0])
)
...
class Agent(nn.Module):
    def __init__(self, mapsize=16 * 16, lstm_hidden=384, lstm_layers=3):
        ...
        self.z_embedding = nn.Embedding(num_embeddings=7, embedding_dim=16)
        self.scalar_encoder  = ScalarFeatureEncoder(11)
        #Scalarencoder()
        self.actor = layer_init(nn.Linear(256+32+ 320, self.mapsize * envsT.action_plane_space.nvec.sum()), std=0.01)
        self.critic = layer_init(nn.Linear(256+32+ 320, 1), std=1)

    def forward(self, x,sc,z):
        sc_feat = self.scalar_encoder(sc)
        z_embed = self.z_embedding(z)

        z_feat = z_embed.view(z.size(0), -1)
    ...
    def bc_loss_fn(self, obs,sc, expert_actions,z):
        ...
        invalid_action_masks = torch.ones((B * H*W, envsT.action_plane_space.nvec.sum()+1), dtype=torch.bool, device=device)
        ...
        mismatches = (action != expert_actions).float()
        ...
        return bc_loss,mismatches.sum().item()



    def get_action(self, x,Sc ,action=None, invalid_action_masks=None, envs=None):
        logits = self.actor(self.forward(x,Sc))
    ...
    def get_value(self, x,Sc):
        return self.critic(self.forward(x,Sc))
...
if BCtraining:
    ...
    opponents =[microrts_ai.passiveAI]#[microrts_ai.workerRushAI, microrts_ai.passiveAI, microrts_ai.lightRushAI, microrts_ai.coacAI, microrts_ai.mayari, microrts_ai.randomAI, microrts_ai.randomBiasedAI]
    all_sc = []
    all_obs = []
    all_act = []
    Latenttorch = torch.zeros((0,20),dtype=torch.long).to(device)
    obsten = torch.zeros((0, 16,16,73), dtype=torch.long).to(device)
    actten = torch.zeros((0, 256, 7), dtype=torch.long).to(device)
    scten = torch.zeros((0,11), dtype=torch.long).to(device)

    Lat = torch.zeros(20,dtype=torch.long).to(device)#0 empty,1 base, 2 barrack, 3 worker, 4 light,5  heavy,6 ranged

    for x in opponents:
        # ai_list = [microrts_ai.coacAI if i % 2 == 0 else opponents[(i // 2) % len(opponents)] for i in range(24)]

        ai_list = [microrts_ai.coacAI, x]
        ...
        for ep in range(Runs):
            ...
            while not dones.all():
                ...
                all_sc.append(getScalarFeatures(obs_batch, res))
                scten = torch.cat([ scten, getScalarFeatures(obs_batch, res)], dim=0)
                ...
                arr2 = np.zeros((256, 7), dtype=np.int64)

                for j in range(len(action[0])):
                    if action[0][j][1] == 4:

                        zer = (Lat[0:10] == 0).nonzero()

                        if len(zer) >0:
                            Lat[zer[0].item() + 0] = action[0][j][6]

                    arr[action[0][j][0]] = action[0][j][1:]

                for j in range(len(action[1])):
                    if action[1][j][1] == 4:
                        zer = (Lat[10:20] == 0).nonzero()

                        if len(zer) >0:
                            Lat[zer[0].item()+10] = action[1][j][6]
            ...
            obsten = torch.cat([obsten,torch.tensor(obs_arr).squeeze(1)], dim=0)
            actten = torch.cat([actten,torch.tensor(act_arr)], dim=0)

            Latenttorch = torch.cat([Latenttorch,Lat.unsqueeze(0).repeat(len(obs_arr),1)], dim=0)
            print(obsten.shape)
            print(actten.shape)
            print(scten.shape)
            print(Latenttorch.shape)
    ...
    sc_ten = torch.cat(all_sc, dim=0)
    print(obs_tensor.shape)
    print(action_tensor.shape)
    print(sc_ten.shape)
    print("agfa")
    obs_tensor = obs_tensor.reshape(-1, 16, 16, 73).to(device)
    action_tensor = action_tensor.reshape(-1, 256, 7).to(device)
    sc_tensor = sc_ten.reshape(-1, 11).to(device)
    print(obs_tensor.shape)
    print(action_tensor.shape)
    print(sc_ten.shape)
    print(obsten.shape)
    ...
    print(scten.shape)
    print(Latenttorch.shape)
    print(torch.equal(obs_tensor,obsten))
    print(torch.equal(action_tensor, actten))
    print(torch.equal(sc_ten, scten))


    dataset = TensorDataset(obsten, actten,scten,Latenttorch)

    train_loader = DataLoader(dataset, batch_size=256, shuffle=True)



    optimizer = optim.Adam(agent.parameters(), lr=1e-5, eps=1e-5)
    ...
    for epoch in range(start_epoch ,num_epochs):
        ...
        for obs_batch, expert_actions, sc, latentv in train_loader:
            ...
            loss, fehler = agent.bc_loss_fn(obs_t, sc_t, expert_t, latentv)
            Mistake += fehler
            optimizer.zero_grad()
            loss.backward()
...
for update in range(starting_update, num_updates + 1):
    ...
    for step in range(0, args.num_steps):
        ...
        obs[step] = next_obs
        ScFeatures[step] = getScalarFeatures(next_obs,res)
        ...
        with torch.no_grad():
            ...
            values[step] = agent.get_value(obs[step],ScFeatures[step]).flatten()
            action, logproba, _, invalid_action_masks[step] = agent.get_action(obs[step],ScFeatures[step], envs=envs)
        ...
        real_action = torch.cat([
            torch.stack(
                [torch.arange(0, mapsize, device=device) for i in range(envsT.num_envs)
    ...
    with torch.no_grad():
        ...
        last_value = agent.get_value(next_obs.to(device),ScFeatures[step]).reshape(1, -1)
        ...
        for t in reversed(range(args.num_steps)):
            ...
            delta = rewards_winloss[t]  + args.gamma * nextvalues * nextnonterminal - values[t] #  rewards_dense[t] + + rewards_score[t]
    ...
    for i_epoch_pi in range(args.update_epochs):
        ...
        for start in range(0, args.batch_size, args.minibatch_size):
            ...
            new_values = agent.get_value(b_obs[minibatch_ind],b_Sc[minibatch_ind]).view(-1)
            if args.clip_vloss:
