...
env = MicroRTSBotGridVecEnv(
    max_steps=2000,
    ais=[microrts_ai.coacAI for _ in range(48)],  # CoacAI plays as second player
...
class Agent(nn.Module):
    def __init__(self, mapsize=16 * 16, lstm_hidden=384, lstm_layers=3):
        ...
        self.encoder = nn.Sequential(
            layer_init(nn.Conv2d(22, 128, kernel_size=3, stride=2, padding=1)),
            nn.ReLU(),
            ResBlock(128),
            ResBlock(128),
            nn.Flatten(),
            layer_init(nn.Linear(128 * 8 * 8, 256)),
            nn.ReLU()
        )

        self.lstm = nn.LSTM(256, lstm_hidden, num_layers=lstm_layers, batch_first=True)
        self.actor = layer_init(nn.Linear(lstm_hidden, mapsize * envs.action_space.nvec[1:].sum()), std=0.01)
    ...
    def forward(self, obs_seq):
        ...
        feats = self.encoder(obs_seq).view(24, 32, -1)  # [B, T, 256]
        lstm_out, _ = self.lstm(feats)  # [B, T, lstm_hidden]
        return lstm_out

    def get_action(self, obs_seq, action=None, invalid_action_masks=None, envs=None):
        lstm_out = self.forward(obs_seq)  # [B, T, H]
        logits = self.actor(lstm_out)  # [B, T, mapsize * action_dim]
...
for ep in range(10):
    ...
    mask_arr = mask_arr.transpose(1, 0, 2, 3)
    mask_arr[:, :, :, :] = 1
    obs_tensor = torch.tensor(np.array(obs_arr), device=device)
    action_tensor = torch.tensor(np.array(act_arr), device=device)
    step_mask = torch.zeros((N, T_max), device=device)

    for i in range(N):
        done_times = np.where(dones_list[i])[0]
        Ti = done_times[0] + 1 if len(done_times) > 0 else T_max
        step_mask[i, :Ti] = 1.0

    obs_for_enc = obs_tensor.permute(1, 0, 2, 3,4)

    flat_states = obs_for_enc.reshape(-1, 16, 16,22)

    invalid_mask_flat = torch.tensor(mask_arr, dtype=torch.float32, device=device)

    flat_expert_actions = action_tensor.reshape(N * T_max, 256, 7)

    step_mask_flat = step_mask.view(N * T_max)
    valid_indices = (step_mask_flat == 1.0).nonzero(as_tuple=True)[0]

    action_pred, logprob_flat, entropy_flat, _ = agent.get_action(
        flat_states,
        action=flat_expert_actions,
        invalid_action_masks=invalid_mask_flat,
        envs=env
    )

    bc_loss = -logprob_flat[valid_indices].mean()
    #bc_loss = - (logprob_sum / 256).mean()


    loss = bc_loss / accum_steps  # <-- scale loss
    loss.backward()
    ...
    print(f"[BC epoch {ep}]  BC_loss = {bc_loss.item():.6f}")
