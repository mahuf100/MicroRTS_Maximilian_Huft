...
from gym_microrts.microrts_ai import passiveAI
...
experiment_name = "MicrortsDefeatCoacAIShaped-v3__te__1__1750972223"
...
class Agent(nn.Module):
    def __init__(self, mapsize=16 * 16, lstm_hidden=384, lstm_layers=3):
        ...
        #Scalarencoder()
        self.actor = layer_init(nn.Linear(256+ 64, self.mapsize * envsT.action_plane_space.nvec.sum()), std=0.01)
        self.critic = layer_init(nn.Linear(256+64, 1), std=1)

    def forward(self, x, sc):
        sc_feat = self.scalar_encoder(sc)
        obs_feat = self.network(x.permute((0, 3, 1, 2)))  # "bhwc" -> "bchw"
        feat = torch.cat([obs_feat, sc_feat], dim=-1)
        return feat
    def bc_loss_fn(self, obs,sc , expert_actions):

        B, H, W, C = obs.shape
        features = self.forward(obs,sc)
...
if BCtraining:
    ...
    N = 1
    Runs = 10
    opponents = [microrts_ai.passiveAI]#[microrts_ai.coacAI, microrts_ai.passiveAI, microrts_ai.lightRushAI, microrts_ai.workerRushAI, microrts_ai.mayari, microrts_ai.randomAI, microrts_ai.randomBiasedAI]
    ...
    for x in opponents:
        ...
        for ep in range(Runs):
            ...
            while not dones.all() and step < T_max:
                ...
                sc_arr.append(np.array(getScalarFeatures(obs_batch,res)))
                obs_batch, mask, rew, dones, action, res  = envT.step("")

                action = java_action_to_python_action_list(action)
    ...
    sc_tensor = torch.tensor(np.concatenate(all_sc, axis=0), device=device).squeeze(1)

    obs_tensor = obs_tensor.reshape(-1,16,16,73).to(device)
    action_tensor = action_tensor.reshape(-1,256,7).to(device)
    sc_tensor = sc_tensor.reshape(-1, 11).to(device)


    dataset = TensorDataset(obs_tensor, action_tensor,sc_tensor)
    ...
    for epoch in range(start_epoch ,num_epochs):
        ...
        for obs_batch, expert_actions, sc in train_loader:
            ...
            loss  = agent.bc_loss_fn(obs_t, sc_t, expert_t)
