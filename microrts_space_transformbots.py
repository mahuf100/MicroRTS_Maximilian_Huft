...
class MicroRTSSpaceTransformbot(VectorEnv, MicroRTSInterfaceListener):
    ...
    def step(self, actions) -> VecEnvStepReturn:
        ...
        microrts_obs, microrts_mask,reward,_, d, r, i, res = self.interface.step("microrts_action")
        ...
        return obs, r, d,i,res,reward

    def reset(self, **kwargs) -> VecEnvResetReturn:
        microrts_obs, microrts_mask,res = self.interface.reset()
        self._update_action_mask(microrts_mask)

        return self._from_microrts_obs(microrts_obs), {},res
    ...
    def reverse_to_microrts_action(self, actions_per_env):
        """
        Reconstruct the original action tensor from processed actions.

        :param actions_per_env: List of actions per environment.
        :param source_unit_idxs: Array of shape (num_envs, num_units, 1) indicating unit positions.
        :param source_unit_mask: Boolean array of shape (num_envs, num_units) indicating valid units.
        :param num_envs: Number of environments.
        :param height: Height of the map.
        :param width: Width of the map.
        :param action_dim: Dimension of each action.
        :return: Reconstructed action tensor of shape (num_envs, height * width, action_dim).
        """
        action_tensor = np.zeros((self.num_envs, self.height * self.width, 8), dtype=np.int32)
        microrts_obs, microrts_mask, _, _, r, d, i = self.interface.step(actions_per_env)
        for env_idx in range(self.num_envs):
            self._update_action_mask(microrts_mask)
            unit_positions = self.source_unit_idxs[env_idx][self.source_unit_mask[env_idx] == 1]
            actions = actions_per_env[env_idx]

            for pos, action in zip(unit_positions, actions):
                action_tensor[env_idx, pos] = action
