...
experiment_name = "MicrortsDefeatCoacAIShaped-v3__ne__1__1750977134"
...
class Agent(nn.Module):
    def __init__(self, mapsize=16 * 16, lstm_hidden=384, lstm_layers=3):
        ...
        selfScalarencoder = ScalarFeatureEncoder(8)
        #Scalarencoder()
        self.actor = layer_init(nn.Linear(256, self.mapsize * envsT.action_plane_space.nvec.sum()), std=0.01)
    ...
    def bc_loss_fn(self, obs, expert_actions):
...
if BCtraining:
    ...
    action_test = torch.tensor(np.array(act_arr), device=device)
    obs_test = torch.tensor(np.array(obs_arr), device=device)
    ...
    for epoch in range(start_epoch ,num_epochs):
        ...
        train_count = 0
        Mistake = 0
        for obs_batch, expert_actions in train_loader:


            obs_t = obs_batch.to(device)
            expert_t = expert_actions.to(device)       # [B,256,7]


            loss,fehler  = agent.bc_loss_fn(obs_t, expert_t)
            Mistake += fehler
            optimizer.zero_grad()
            loss.backward()
