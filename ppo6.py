...
class Agent(nn.Module):
    def __init__(self, mapsize=16 * 16, lstm_hidden=384, lstm_layers=3):
        ...
        self.network = nn.Sequential(
            layer_init(nn.Conv2d(22, 32, kernel_size=3, stride=2, padding=1)),
            nn.ReLU(),
            ResBlock(32),
            ResBlock(32),
            nn.Flatten(),
    ...
    def get_action(self, x, action=None, invalid_action_masks=None, envs=None):
        ...
        print(split_logits[0].shape)
        print(split_logits[1].shape)
        print(split_logits[2].shape)
        print(split_logits[3].shape)
        print(split_logits[4].shape)
        print(split_logits[5].shape)

        if action is None:
            print(envs.debug_matrix_mask(0).shape)
            invalid_action_masks = torch.tensor(np.array(envs.debug_matrix_mask(0))).to(device)
...
obss_batch = envsT._from_microrts_obs(ob[0])
print(obss_batch.shape)
...
for update in range(starting_update, num_updates + 1):
    ...
    for step in range(0, args.num_steps):
        time.sleep(0.1)
        obs_batch, mask, rew, rew2, dos, ios, ation = env.step("")
        #time.sleep(1)
        ...
        try:
            ...
            next_obs, denserew, winlossrew,scorerew , ds, infos = envs.step(ation)
