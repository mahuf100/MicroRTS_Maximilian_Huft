...
experiment_name = "MicrortsBC__nurWRCOACMAYA__1__1753532835"
...
envs = MicroRTSGridModeVecEnv(
    ...
    max_steps=2000,
    render_theme=1,
    ai2s=[microrts_ai.coacAI for _ in range(args.num_bot_envs-19)] +[microrts_ai.mayari for _ in range(5)] +[microrts_ai.randomAI for _ in range(2)]+ [microrts_ai.lightRushAI for _ in range(3)]+[microrts_ai.passiveAI for _ in range(3)]+[microrts_ai.workerRushAI for _ in range(3)]+[microrts_ai.randomBiasedAI for _ in range(3)],
...
class Agent(nn.Module):
    def __init__(self, mapsize=16 * 16, lstm_hidden=384, lstm_layers=3):
        ...
        self.z_embedding = nn.Embedding(num_embeddings=3, embedding_dim=8)
    ...
    def bc_loss_fn(self, obs,sc, expert_actions,z):
        ...
        for cat in multi_categoricals:
            probs = cat.probs  # tensor, masked and normalized
            logits = torch.log(probs + 1e-8)  # safe conversion to logits
            masked_cat = cat  # has cat.logits and cat.masks
...
if args.prod_mode and wandb.run.resumed:
    ...
    supervised_agent.load_state_dict(torch.load(ckpt_path, map_location=device, weights_only= True))
...
if BCtraining:
    if newdata:
        ...
        T_max = 2048
        N = 1
#[microrts_ai.workerRushAI,microrts_ai.izanagi],[microrts_ai.coacAI,microrts_ai.izanagi],[microrts_ai.mayari,microrts_ai.izanagi],[microrts_ai.workerRushAI,microrts_ai.droplet],[microrts_ai.coacAI,microrts_ai.droplet],[microrts_ai.mayari,microrts_ai.droplet],
        ...
        for l,x in enumerate(opponents):
            ...
            for ep in range(num_runs[l]):
                ...
                if (ep + 1) % 50 == 0:
                    with open(f"winreplays/replay_{l}_up_to_ep{ep + 1}.pt.zst", 'wb') as f:
    ...
    all_files = sorted([
        os.path.join("winreplays2/", f)
        for f in os.listdir("winreplays2/")
    ...
    for epoch in range(start_epoch, start_epoch + 1000):
        ...
        lr = 1e-4 * (0.6 ** ((epoch - start_epoch) / 1))
        ...
        for obs, expert_actions, sc, zt in train_loader:
            ...
            train_loss_sum += float(loss) * obs.size(0)
...
for update in range(starting_update, num_updates + 1):
    ...
    for step in range(0, args.num_steps):
        ...
        for i in range(args.num_envs):
            with torch.no_grad():
                zFeatures[step][i] = agent.z_encoder(ScFeatures[step][i])
    ...
    with torch.no_grad():
        ...
        for t in reversed(range(args.num_steps)):
            ...
            delta =  rewards_winloss[t]   + args.gamma * nextvalues * nextnonterminal - values[t] #rewards_dense[t] + + rewards_dense[t] +rewards_score[t]
    ...
    for i_epoch_pi in range(args.update_epochs):
        ...
        for start in range(0, args.batch_size, args.minibatch_size):
            ...
            with torch.no_grad():  # freeze SL policy
            ...
            kl_div = torch.mean(sl_logprobs - newlogproba)  # D_KL(π_SL || π_RL)
